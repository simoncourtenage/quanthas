...
data DayCounter
    = DayCounter
        {
            dcDayCount :: CalDate -> CalDate -> Int,
            dcYearFraction :: CalDate -> CalDate -> CalDate -> CalDate -> Time,
            dcName :: String
        }

-- constructor function for day counter objects
makeDayCounter :: (CalDate -> CalDate -> Int) -> (CalDate -> CalDate-> CalDate -> CalDate -> Time) -> String -> DayCounter
...
dayCount :: DayCounter -> CalDate -> CalDate -> Int
dayCount dc d1 d2 = (dcDayCount dc) d1 d2

yearFraction :: DayCounter -> CalDate -> CalDate -> CalDate -> CalDate -> Time
...
defaultDayCount :: CalDate -> CalDate -> Int
...
actualyrfraction :: (CalDate -> CalDate -> Int) -> Double -> CalDate -> CalDate -> CalDate -> CalDate -> Time
...
thirty360dayCountImplUS :: CalDate -> CalDate -> Int
thirty360dayCountImplUS (CalDate d1 m1 y1 s1) (CalDate d2 m2 y2 s2)
...
thirty360dayCountImplEU :: CalDate -> CalDate -> Int
thirty360dayCountImplEU (CalDate d1 m1 y1 s1) (CalDate d2 m2 y2 s2)
    = 360* (y2 - y1) + 30 * (m2 - m1 - 1) + (max 0 (30-d1)) + (min 30 d2)

thirty360dayCountImplItal :: CalDate -> CalDate -> Int
thirty360dayCountImplItal (CalDate d1 m1 y1 s1) (CalDate d2 m2 y2 s2)
